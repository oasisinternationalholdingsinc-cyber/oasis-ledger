 ...
 * ❌ NO hardcoded domain cabinet lists (only optional icon map)
...
type EntryWithDoc = MinuteBookEntry & {
  // primary doc resolved from supporting_documents
...
/* ---------------- data loaders (LOCKED to Supabase schema) ---------------- */
...
async function loadSupportingDocs(entryIds: string[]): Promise<SupportingDoc[]> {
  ...
  // Pull all docs for these entries; we'll pick "primary" client-side by version/uploaded_at
  const { data, error } = await sb
 ...
 * - if there is a verified/official doc referencing this entry/hash/path, prefer that
...
export default function MinuteBookClient() {
  ...
  // Load domains (global, not entity-specific)
  useEffect(() => {
    let alive = true;
    ...
    (async () => {
      try {
        ...
        if (!alive) return;
        // domains failure should not crash page, but we show it
        setDomains([]);
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        // keep domain selection stable; if current domain doesn't exist anymore, reset to all
  ...
  // Domain counts (from actual entries, not invented)
            ...
            <div className="grid grid-cols-12 gap-6 flex-1 min-h-0">
              ...
              <section className="col-span-12 lg:col-span-3 min-h-0 flex flex-col">
                <div className="bg-slate-950/40 border border-slate-800 rounded-2xl p-4 flex flex-col min-h-0">
                  ...
                  {/* mac-like pill list (scroll inside only) */}
                  ...
                  <div className="mt-3 text-[10px] text-slate-500 flex items-center justify-between">
                    <span>Entity: <span className="text-slate-300">{entityKey}</span></span>
                    <Link href="/ci-archive" className="text-slate-400 hover:text-slate-200">Launchpad</Link>
                  </div>
                </div>
              </section>
              ...
              {/* RIGHT: Evidence (PDF + Metadata Zone) */}
                ...
                <div className="bg-slate-950/40 border border-slate-800 rounded-2xl p-4 flex flex-col min-h-0">
                  ...
                  {!selected ? (
                    ...
                    <>
                      ...
                      {/* PDF Preview (scroll stays within right pane) */}
                      ...
                      <div className="mt-3 rounded-xl border border-slate-800/80 bg-slate-950/60 p-3 shrink-0">
                        ...
                        <div className="mt-3 flex items-center justify-between text-[10px] text-slate-500">
                          <Link href="/ci-archive" className="hover:text-slate-200">Archive Launchpad</Link>
