...
export default function MinuteBookClient() {
  ...
  useEffect(() => {
    ...
    if (!selectedId || !filtered.some((e) => e.id === selectedId)) setSelectedId(filtered[0].id);
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5">
          <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3">
            <div className="min-w-0">
              ...
              <div className="mt-1 text-xs text-white/55">Domains → Entries → Details · Non-destructive · PDF preview on demand</div>
            </div>
      ...
      <div className="mx-auto max-w-[1600px] px-5 pb-8 pt-4">
        {!entityKey ? (
          <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
            ...
            <div className="mt-2 text-sm text-white/65">Select an entity from the OS bar (top right).</div>
          </div>
        ) : (
              ...
              <div className="p-2">
                ...
                {loading ? (
                  ...
                  <div className="space-y-1">
                    {domainRail.map((d) => (
                      <RailItem key={d.key} label={d.label} count={d.count} active={domain === d.key} onClick={() => setDomain(d.key)} />
                    ))}
                  </div>
                )}
              </div>
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5">
              <div className="flex items-center justify-between gap-3 px-4 py-3">
                <div>
                  ...
                  <div className="mt-1 text-[11px] text-white/45">{loading ? "Loading…" : `${filtered.length} record${filtered.length === 1 ? "" : "s"}`}</div>
                </div>
                ...
                {loading ? (
                  ...
                  <div className="p-5">
                    <div className="rounded-2xl border border-white/10 bg-black/20 p-5">
                      ...
                      <div className="mt-1 text-sm text-white/60">No minute book entries exist for this scope yet. Upload is the only write entry point.</div>
                      <a
                    ...
                    {filtered.map((e) => {
                      ...
                      return (
                        <button
                          ...
                          className={cls("w-full text-left transition", "border-b border-white/10", "hover:bg-white/5", active && "bg-yellow-500/10")}
                        >
                          <div className="flex items-center justify-between gap-3 px-4 py-3">
                            <div className="min-w-0">
                              <div className="flex items-center gap-2">
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5">
              ...
              <div className="max-h-[72vh] overflow-auto p-4">
                {loading ? (
                  ...
                  <div className="rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-white/65">Select an entry to view details.</div>
                ) : (
                      ...
                      <div className="rounded-2xl border border-yellow-500/20 bg-yellow-500/5">
                        ...
                        >
                          <div>
                            ...
                            <div className="mt-0.5 text-[11px] text-yellow-100/55">{previewOpen ? "Collapse PDF preview" : "Open PDF preview"}</div>
                          </div>
                        ...
                        {previewOpen ? (
                          <div className="border-t border-yellow-500/15 p-4">
                            {previewBusy ? (
                              <div className="rounded-xl border border-white/10 bg-black/20 p-3 text-sm text-white/65">Preparing preview…</div>
                            ) : previewUrl ? (
...
function RailItem({ label, count, active, onClick }: { label: string; count: number; active: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className={cls("flex w-full items-center justify-between gap-3 rounded-xl px-3 py-2 text-left transition", active ? "bg-yellow-500/10" : "hover:bg-white/5")}
    >
...
function Badge({ children }: { children: React.ReactNode }) {
  return <span className="rounded-full border border-white/10 bg-black/20 px-2 py-0.5 text-[11px] text-white/70">{children}</span>;
}

function GoldBadge({ children }: { children: React.ReactNode }) {
  return <span className="rounded-full border border-yellow-500/20 bg-yellow-500/10 px-2 py-0.5 text-[11px] text-yellow-100/80">{children}</span>;
}
...
function VaultIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" className="text-yellow-200">
      <path d="M4 7a3 3 0 0 1 3-3h10a3 3 0 0 1 3 3v10a3 3 0 0 1-3 3H7a3 3 0 0 1-3-3V7Z" stroke="currentColor" strokeWidth="1.6" />
...
function PaperclipIcon() {
  return (
    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" className="text-white/70">
      <path d="M21 10.5 12.5 19a5 5 0 0 1-7-7l9-9a3.5 3.5 0 0 1 5 5l-9 9a2 2 0 1 1-3-3l8.5-8.5" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
function SearchIcon() {
...
function SealIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" className="text-yellow-200">
      <path d="M12 2l2.2 4.4L19 7l-3.5 3.4.8 4.8L12 13.8 7.7 15.2l.8-4.8L5 7l4.8-.6L12 2Z" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round" />
...
function LinkIcon() {
  return (
    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" className="text-white/70">
      <path d="M10 13a5 5 0 0 1 0-7l1-1a5 5 0 0 1 7 7l-1 1" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
      <path d="M14 11a5 5 0 0 1 0 7l-1 1a5 5 0 0 1-7-7l1-1" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
    </svg>
  );
}
