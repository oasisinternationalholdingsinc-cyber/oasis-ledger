...
export default function MinuteBookClient() {
  ...
  useEffect(() => {
    const fromUrl =
      sp.get("entity_key") || sp.get("entityKey") || sp.get("entity");
    if (fromUrl) {
  ...
  const counts = useMemo(() => {
    ...
    for (const e of entries)
      m.set(e.domain_key, (m.get(e.domain_key) || 0) + 1);
    return m;
  }, [entries]);

  const domainRail = useMemo(() => {
    const canonicalKeys = new Set(CANONICAL_DOMAINS.map((d) => d.key));
    const dynamic = Array.from(counts.keys()).filter(
      (k) => k && !canonicalKeys.has(k)
    );
    ...
    for (const k of dynamic)
      items.push({ key: k, label: titleize(k), count: counts.get(k) || 0 });
  ...
  const filtered = useMemo(() => {
    const base =
      domain === "all" ? entries : entries.filter((e) => e.domain_key === domain);
  ...
  // If selection changes, close preview
  useEffect(() => {
    setPreviewOpen(false);
    setPreviewUrl(null);
    setPreviewErr(null);
        ...
        {!entityKey ? (
          <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
            ...
            <div className="mt-2 text-sm text-white/65">
              Your OS entity selector should append{" "}
              <span className="text-white/85">?entity_key=...</span> to the URL.
            </div>
          </div>
        ) : (
                  ...
                  <div className="p-5">
                    <div className="rounded-2xl border border-white/10 bg-black/20 p-5">
                      ...
                      <div className="mt-1 text-sm text-white/60">
                        No minute book entries exist for this scope yet. Upload is
                        the only write entry point.
                      </div>
                      <a
                        href={`/ci-archive/upload?entity_key=${encodeURIComponent(
                          entityKey
                        )}`}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
                      ...
                      <div className="mt-4 grid grid-cols-2 gap-2">
                        ...
                        <Meta
                          ...
                          value={fmtDate(selected.instrument_date)}
                        />
                    ...
                    <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
                      ...
                      <div className="mt-2 whitespace-pre-wrap text-sm text-white/80">
                        {selected.notes ? (
                          ...
                          <span className="text-white/45">No notes.</span>
                        )}
                      </div>
                    </div>
                    ...
                    {selected.storage_path && isPdf(selected.file_name) ? (
                      <div className="rounded-2xl border border-yellow-500/20 bg-yellow-500/5">
                        ...
                        >
                          <div>
                            <div className="text-xs font-semibold tracking-wide text-yellow-100/80">
                              PREVIEW
                            </div>
                          ...
                          <span
                            className={cls(
                              "transition",
                              previewOpen ? "rotate-90" : "rotate-0"
                            )}
                          >
                            <ChevronRight />
                          </span>
                      ...
                      <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
                        ...
                        <div className="mt-3 space-y-2">
                          {selected.source_record_id ? (
                            <a
                              href={`/ci-archive/ledger?entity_key=${encodeURIComponent(
                                entityKey
                              )}&source_record_id=${encodeURIComponent(
                                selected.source_record_id
                              )}`}
                          ...
                          {selected.source_envelope_id ? (
                            <a
                              href={`/ci-forge?envelope_id=${encodeURIComponent(
                                selected.source_envelope_id
                              )}`}
                              className="flex items-center justify-between rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white/85 hover:bg-white/10"
                            >
...
function Meta({
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-black/20 p-3">
      ...
      <div className={cls("mt-1 text-xs text-white/80", mono && "break-all font-mono")}>
        {value}
      </div>
    </div>
  );
