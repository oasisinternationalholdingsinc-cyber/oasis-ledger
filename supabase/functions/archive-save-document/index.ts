...
type ReqBody = {
  ...
  is_test?: boolean;      // optional; we will derive from governance_ledger for truth
};
...
async function getActorUid(req: Request): Promise<string | null> {
  // Prefer caller JWT so uploaded_by/owner_id can be set to a real user
...
async function rpcSeal(ledgerId: string) {
  // Your RPC signature is p_ledger_id uuid
  return await supabase.rpc("seal_governance_record_for_archive", { p_ledger_id: ledgerId });
}

async function rpcVerify(ledgerId: string) {
  // Your RPC signature is p_ledger_id uuid
  return await supabase.rpc("verify_governance_archive", { p_ledger_id: ledgerId });
}
...
serve(async (req) => {
  ...
  try {
    ...
    // 1) Load ledger (we derive is_test from ledger truth)
    ...
    const ledger_is_test = asBool((gl.data as any).is_test, false);
    ...
    // 2) Load envelope (must be completed and must match record_id)
    ...
    const envIsTest = asBool((env.data as any).is_test, false);
    ...
    // Lane truth is ledger_is_test (envelope mismatch is a warning, not a blocker)
    const is_test = ledger_is_test;
    const lane_warning = envIsTest !== ledger_is_test
      ? { warning: "envelope.is_test != ledger.is_test (keeping ledger lane as truth)", env_is_test: envIsTest, ledger_is_test }
      : null;

    // 3) Resolve entity_key from entity_companies
    ...
    if (!entity_key) {
      return json(
        { ok: false, error: "Unable to resolve entity_key (entity_companies lookup failed).", details: ec.error ?? null },
        500
      );
    }

    // 4) Find/create minute_book_entries (idempotent by source_record_id + is_test)
    ...
    // Choose a domain + default section
    ...
    if (!domain_key) {
      return json({ ok: false, error: "No active governance_domains found (seed them first)." }, 500);
    }
    ...
    if (!default_section) {
      return json(
        { ok: false, error: "Missing governance_domain_sections.default_section for domain.", domain_key },
        500
      );
    }
    ...
    if (!minute_book_entry_id) {
      ...
      if (ins.error || !ins.data) {
        return json({ ok: false, error: "Failed to create minute_book_entries", details: ins.error ?? null }, 500);
      }
      minute_book_entry_id = (ins.data as any).id as string;
    }

    // 5) Seal (service_role ok) — returns jsonb
    const seal = await rpcSeal(record_id);
    if (seal.error) {
    ...
    if (!storage_bucket || !storage_path || !file_hash) {
      return json(
        {
          ok: false,
          error: "Seal did not return storage_bucket/storage_path/file_hash",
          details: sealed,
        },
        500
      );
    ...
    // 6) Ensure PRIMARY supporting_documents pointer exists (idempotent)
    ...
    if (!sdExisting.data) {
      ...
      if (sdIns.error) {
        return json({ ok: false, error: "Failed to create supporting_documents pointer", details: sdIns.error }, 500);
      }
    }

    // 7) Patch minute_book_entries pointers (best effort)
    ...
    // 8) Verify + read verified_documents
    const verify = await rpcVerify(record_id);
    const verify_json = verify.data ?? null;
